...
### Row 1 — Deal Analyzer, 2026-08-04 · `feedback #1`

> *"lookin good"*

**No ask** — and that is a judgement, so it is recorded as one rather
than inferred by a heuristic. A row with nothing in it is claimed exactly
like any other.

### Row 2 — Site DD, 2026-08-16 23:28, from assessment 11 · `feedback #2`
...
### Row 3 — Investor Report, 2026-08-16 23:39, from deal 2 · `feedback #3`
...
> **THE CLASS HAS GROWN AND THE HEADING UNDERSTATES IT, measured
> 2026-09-09.** The container now skips **29** of 2,697 against local's
> **3**. Nineteen are still the T12s; the rest are the same arrangement
> arriving in newer modules — `test_appfolio_rent_roll.AgainstTheRealFile`
> and, since Part 116, the five in `test_sitedd_bed_note` that drive the
> real ResMan and Appfolio rolls through the seeding path. **All of them
> ran here, on the machine that has the files.**
>
> **The number is bookkeeping; the shape is not.** Every one of these
> gates exists because a client's file may not go in the repo, and each
> new one widens the set of behaviour that *"run it on the container"*
> does not reach. That is the right trade and it should be made
> deliberately rather than discovered — **a test gated on a path under
> somebody's Downloads folder is verified on one laptop, and the entry
> above is the standing reminder to say so out loud when adding one.**
> Part 116 says so: the deployed check for the bed note was driven from
> here, with the files streamed through the real upload route on the
> container, precisely because the container's own suite skips it.
...
## What the feedback table holds

*Generated by `tools/feedback_digest.py` — regenerate it, never edit it.
A row is CLAIMED when something in the repo cites it as `feedback #N`;
that is the whole convention, and it is deliberately one string. An
unread flag is cleared by a glance, and looking is not what was missing.*

<!-- FEEDBACK:BEGIN -- generated by tools/feedback_digest.py, do not edit -->

**4 rows. 0 unclaimed.** A row is claimed when something in the repo cites it — write `feedback #N` in a comment, a test, or here. A row with nothing to do is claimed the same way: say so and cite it.

| # | when | tool | first line | items | claimed by |
|---|---|---|---|---|---|
| **1** | 2026-08-04 | Deal Analyzer | lookin good | — | `HANDOFF.md` |
| **2** | 2026-08-16 | Site DD | 1. Swap out the condition summary for the following fields: | 2 | `HANDOFF.md` |
| **3** | 2026-08-16 | Investor Report | 1. remove top description | 4 | `tests/test_investor_notes.py`, `HANDOFF.md` |
| **4** | 2026-08-31 | Weekly Property Summary | Hi Jasper! I tested the info overall it looks good - I know that you are starting school s… | — | `tools/mmr_summary.py`, `HANDOFF.md`, `docs/site-dd-per-bed-occupancy.md` |
